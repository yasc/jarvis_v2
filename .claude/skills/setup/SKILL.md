...
- Record PLATFORM value for later steps
...
This builds the agent-runner subprocess (installs its dependencies and compiles TypeScript).

**If BUILD_OK=false:** Read `logs/setup.log` tail for the build error.
- If TypeScript compilation error: read the error, diagnose and fix the source
- If npm install failed: delete `container/agent-runner/node_modules` and `container/agent-runner/package-lock.json`, retry
- If missing build tools: install them (`xcode-select --install` on macOS, `build-essential` on Linux), then retry
...
5. Pick the most likely candidates and present them as AskUserQuestion options — show names only, not JIDs.
...
- AGENT_RUNNER=not_built → re-run step 3.
...
**Service not starting:** Check `logs/nanoclaw.error.log`. Common causes: wrong Node path in plist (re-run step 9), missing `.env` (re-run step 4), missing WhatsApp auth (re-run step 5).

**Agent fails ("process exited with code 1"):** Check agent logs in `groups/main/logs/agent-*.log`. Common causes: missing Claude credentials in `.env`, agent-runner not built (run `npm run build`).
